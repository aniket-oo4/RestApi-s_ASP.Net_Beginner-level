...
namespace LeaveData_DL
{
    public class DataManager
    {
        ...
        public List<LeaveModel> GetApplications()
        {

            List<LeaveModel> applications = new List<LeaveModel>();
            using (SqlConnection conn = new SqlConnection(_connectionstring))
            {
                conn.Open();
                string query = "select E.emp_id,E.Emp_name ,L.leave_type ,L.totalLeaves Days,Format(L.ApplicationDate,'dd MMMM yyyy') AS ApplicationDate,Format(L.leave_dateFrom,'dd MMM yyyy') AS  startDate,Format(L.leave_dateTo,'dd MMM yyyy') AS endDate from Employee E join Leaves L on L.emp_id=E.emp_id";
                SqlCommand cmd = new SqlCommand(query, conn);
                SqlDataReader dataReader = cmd.ExecuteReader();
                while (dataReader.Read())
                {
